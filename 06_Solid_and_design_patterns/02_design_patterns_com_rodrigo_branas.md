...
[Code](<02_design_patterns_com_rodrigo_branas_source/v8_controller>)

> Conecta o driver com a aplicação, repassando os dados de entrada e retorno a saída de acordo com o drive

Sendo assim precisamos centralizar em um MainController a responsabilidade de definir rotas. De forma com ele nao conheca o HttpServer exato e o Use Case extato.
...
No caso do HttpServer estamos colocando o outro pattern Adapter em cena. Criando a interface HttServer e o Adapter do express (Servidor web especifico sendo utilizado)
...
O que falta agora é criar um main (entrypoint)
...
Que já um padrao chamado `Composition root`
...
Ele quem vai saber quem sao as instancias exatas que minha applicacao está utilizando

Alem disso podemos organizar a estrutura da aplicacao da seguinte forma
...
[Code](<02_design_patterns_com_rodrigo_branas_source/v9_mediator)

> Cria um mecanismo de notificação para reduzir o acoplamento entre os objetos

Resolve o classico UseCase precisa chamar outro UseCase.

É parecido com o Observer que geralmente é 1 pra 1

Vamos imaginar que apos gerar o invoice precisar enviar um email, funcionalidade feita por outro useCase. Para evitar acoplamento e inclusive o risco de dependencia ciclica.

O mediator sera um event listener, em que algum lugar eles deve ser registrados

Podemos criar um mediator que possui uma array de observers, com a string chave do evento e o callback a ser utilizado.

Com um metodo `on` que servirá para registrar, inserirmos na lista o evento com sua chave e seu callback, e o metodo publish, que procura a chave passada na lista para executar o callback
...
Assim na hora do fim do nosso usecase (generateInvoice) podemos chamar o publish para que o mediator injetado faca essa chamada do callback
...
Para isso, na main fazemos tanto o registro no mediator do execute do novo use case, quanto a injecao do mediator no use case chamador
...
<details><summary>Resumo do capitulo</summary>
